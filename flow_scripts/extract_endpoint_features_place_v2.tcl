...
# FF neighborhood count within radius R (microns) among endpoints
# Precompute endpoint centers, then count by brute-force (ok for <= ~100k FF; typical blocks much less)
proc count_neighbors {cx cy radii centers} {
    # centers is list of {x y}
    array set res {}
    foreach r $radii { set res($r) 0 }
    ...
    foreach xy $centers {
        ...
        foreach r $radii {
            if {$d2 <= $r*$r && $d2 > 0.0} { incr res($r) }
        }
    }

    set out {}
    foreach r $radii { lappend out $res($r) }
    return $out
}
...
foreach ep $eps {
    ...
    # Neighborhood FF density
    lassign [count_neighbors $cx $cy $neigh_radii $centers] neigh_counts
